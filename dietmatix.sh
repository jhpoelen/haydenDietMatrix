...
zcat interactionsPreyPred.tsv.gz | nomer append --properties=my.properties | grep SAME_AS | awk -F '\t' '{ print $3 "\t" $4 "\t" $1 "\t" $2 "\t" $6 "\t" $7}' | grep -P "^FBC:FB" | grep -v -P "\t\t$" | gzip > fbPredPreyOrderUnmapped.tsv.gz
